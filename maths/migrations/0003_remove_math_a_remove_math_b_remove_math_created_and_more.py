# Generated by Django 5.2.1 on 2025-06-05 16:39

import django.db.models.deletion
from django.db import migrations, models


class Migration(migrations.Migration):

    dependencies = [
        ('maths', '0002_alter_math_operation'),
    ]

    operations = [
        migrations.RemoveField(
            model_name='math',
            name='a',
        ),
        migrations.RemoveField(
            model_name='math',
            name='b',
        ),
        migrations.RemoveField(
            model_name='math',
            name='created',
        ),
        migrations.RemoveField(
            model_name='math',
            name='operation',
        ),
        migrations.CreateModel(
            name='Result',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('value', models.FloatField(blank=True, null=True, unique=True)),
                ('error', models.CharField(blank=True, max_length=255, null=True, unique=True)),
            ],
            options={
                'constraints': [models.CheckConstraint(condition=models.Q(models.Q(('error__isnull', True), ('value__isnull', False)), models.Q(('error__isnull', False), ('value__isnull', True)), _connector='OR'), name='maths_result_value_error_together')],
            },
        ),
        migrations.AddField(
            model_name='math',
            name='result',
            field=models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, to='maths.result'),
        ),
    ]
